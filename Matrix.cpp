...
namespace dae {
	...
	Matrix Matrix::CreateLookAtLH(const Vector3& origin, const Vector3& forward, const Vector3& up)
	{
		...
		return  Matrix{
		Vector4{xAxis.x, yAxis.x, forward.x, 0},
		Vector4{xAxis.y, yAxis.y, forward.y, 0},
		Vector4{xAxis.z, yAxis.z, forward.z, 0},
		Vector4{-Vector3::Dot(xAxis, origin), -Vector3::Dot(yAxis, origin), -Vector3::Dot(forward, origin), 1} };
	}

	Matrix Matrix::CreatePerspectiveFovLH(float fov, float aspect, float zn, float zf)
	{
		return Matrix{ Vector4{1.f / (fov * aspect),0,0,0},
